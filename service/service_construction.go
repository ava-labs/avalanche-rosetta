...
func (s ConstructionService) ConstructionMetadata(
	...
	var input options
	if err := types.UnmarshalMap(req.Options, &input); err != nil {
	...
	metadataMap, err := types.MarshalMap(metadata)
	if err != nil {
...
func (s ConstructionService) ConstructionParse(
	...
	metaMap, err := types.MarshalMap(metadata)
	if err != nil {
		return nil, wrapError(errInternalError, err)
	}
...
func (s ConstructionService) ConstructionPayloads(
	...
	if err := types.UnmarshalMap(req.Metadata, &metadata); err != nil {
		return nil, wrapError(errInvalidInput, err)
	}
...
func (s ConstructionService) ConstructionPreprocess(
	...
	marshaled, err := types.MarshalMap(preprocessOptions)
	if err != nil {
		return nil, wrapError(errInternalError, err)
	}
